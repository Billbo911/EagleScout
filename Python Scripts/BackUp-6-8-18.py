...
StrToInt_dict = {'Team': int, 'Match_Num': int, 'Start_lvl': int, 'A_R_C': int, 'A_R_C_F': int,
                 'A_R_H': int, 'A_R_H_F': int, 'A_C_C': int, 'A_C_C_F': int, 'A_C_H': int, 'A_C_H_F': int,
                 'T_R_C': int, 'T_R_C_F': int, 'T_R_H': int, 'T_R_H_F': int, 'T_C_C': int, 'T_C_C_F': int,
                 'T_C_H': int, 'T_C_H_F': int, 'Drvr_Perf': int, 'Auto_Perf': int, 'Climb_Lvl': int, 'Name': str,
                 'Comments': str, 'Point_Contrib': int}
...
for sht in Teams:
    ...
    sn.cell(row=21, column=6).value = "=SUM(F16+N16)"
    sn.cell(row=22, column=6).value = "=SUM(J16+R16)"
    sn.cell(row=23, column=6).value = "=SUM(D16+L16)"
    sn.cell(row=24, column=6).value = "=SUM(H16+P16)"
...
Stuff = ['Team #', 'Av Roc Hatch',  'Av Carg Hatch', 'Av Roc Cargo', 'Av Carg Cargo', 'Av Climb', 'Av Point Contrib']
s = 0
for st in Stuff:
...
SN.conditional_formatting.add('B2:F75',
                              CellIsRule(operator='greaterThan', formula=[2.99], stopIfTrue=True, fill=greenFill))
SN.conditional_formatting.add('B2:F75',
                              CellIsRule(operator='between', formula=[.99, 2.98], stopIfTrue=True, fill=yellowFill))
SN.conditional_formatting.add('B2:F75',
                              CellIsRule(operator='lessThan', formula=[.98], stopIfTrue=True, fill=redFill))
...
df1 = pd.read_excel('/EagleScout/Python Scripts/Match_Schedule/Match_Schdule.xlsx', sheet_name='Match Schedule',
                    header=0)
df2 = pd.read_excel('/EagleScout/Python Scripts/Match_Schedule/Match_Schdule.xlsx', sheet_name='2073 Schedule',
                    header=0)
...
MS.to_csv('./Spreadsheets/Match_Schdule.csv',
          columns=('Match #', 'Red 1', 'Red 2', 'Red 3', 'Blue 1', 'Blue 2', 'Blue 3'), index=False)
